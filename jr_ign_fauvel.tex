...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgcee23c9}]{OCS \& télédétection}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgadc16d6}]{CES OSO}
\begin{center}
...
\begin{frame}[label={sec:orgc08fb25}]{Nomenclature actuelle}
\begin{columns}
\begin{column}{0.5\columnwidth}
\small
\begin{itemize}
...
\begin{frame}[label={sec:orgcd8fc59}]{OCS France pour 2016}
\begin{center}
\includegraphics[width=\linewidth]{./figures/OSO_V4.png}
\end{center}
\end{frame}

\begin{frame}[fragile,label={sec:orgf15a77c}]{Précision de classification}
\scriptsize
\pgfplotstableset{
    color cells/.style={
      col sep=comma,
...
\label{sec:org6d1d8bd}
\begin{frame}[label={sec:orgcb7567f}]{Étapes de traitements}
  \begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf6ffa9f}]{Extraction des données d'entraînement/validation}
\begin{onlyenv}<1>
\begin{center}
...
\begin{frame}[label={sec:org848bf2c}]{Classification}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org8b00dee}]{}
\end{frame}
\end{document}
